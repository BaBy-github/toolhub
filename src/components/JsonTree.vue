...
function typeOf(v: any) {
  return isArr(v) ? 'array' : isObj(v) ? 'object' : typeof v
}
function toggle(path: string) {
  open.value[path] = !open.value[path]
}
function entries(v: any) {
  return isObj(v) ? Object.entries(v) : isArr(v) ? v.map((x, i) => [String(i), x]) : []
}
    ...
    const head: any = () =>
      h('div', { class: 'flex items-center gap-2' }, [
        clickable.value
          ? h(
              'button',
              { class: 'btn-ghost px-2 py-1', onClick: () => toggle(props.path) },
              opened.value ? '−' : '+',
            )
          : null,
        h(
          'span',
          { class: 'text-gray-700' },
          t.value === 'object' ? '{}' : t.value === 'array' ? '[]' : String(props.v),
        ),
        ...
        ? h(
            ...
            entries(props.v).map(([k, val]: any) =>
              h('div', { class: 'flex flex-col gap-1' }, [
                h('div', { class: 'flex items-center gap-2' }, [
                  h('span', { class: 'text-blue-600 text-xs' }, k + ':'),
                ]),
